...
- `project.language` 只控制最终学术产物的语言和参考规则选择。任务包中的对应字段是 `output_language`：`zh` 产出中文学术内容，`en` 产出英文学术内容；任务包缺少 `output_language` 时按 `zh` 处理。
- 用户交互、CLI 帮助、错误和确认提示默认使用简体中文，不随 `project.language` 改变。
- 把 `taskpack.reference_inputs` 视为写作、审阅和修订的质量规则。传递任务时，应说明这些文件是框架规则，不是用户提供的事实证据。
- 当 `project.type: paper` 时，`taskpack.reference_inputs` 会额外选择论文 lifecycle、section writing、style routing、review gates、venue/visual、English polishing 和 citation strategy references。科研选题顾问与引用格式 helper 不在主链内，继续按 handoff 处理。
- 调用专门角色前必须检查 `reference_inputs`。先读取 `required`，再读取 `section_specific`，上下文预算允许时再读取 `optional`。
- `inputs` 和 `outputs` 中的路径相对用户写作项目解释；`reference_inputs.path` 相对 ScholarWriting runtime 解释。
...
4. 按返回的 `action` 推进：
   ...
   - `run_writer`：让写作角色按 `output_language` 完成 `sections/` 中的目标章节。
   - `run_reviewers`：让审阅角色按 `output_language` 生成审阅报告；只有在用户要求多角色审阅或任务确实受益时，才并行运行独立审阅维度。
   - `run_revision`：让修订角色按 `output_language` 处理审阅意见，并严格限制在任务包允许的写入边界内。
...
在 ScholarWriting 源码仓库内开发时，本 repo-local skill 是平台中性的调试入口。Codex 可以配合源码中的 `.codex/agents` 和源码 CLI 调试；其他能发现 `.agents/skills` 的框架也应按各自原生机制消费同一套任务包。
