...
1. `scholar_writing/resources/config/default_config.yaml` — 框架默认值
...
当前 controller 只消费 `section_score_threshold`。其余字段已进入配置 schema，但尚未改变状态推进；下面的全文收敛条件是目标设计。
...
当前 controller 只消费 `score_weights.section`；`score_weights.global` 是预留配置。
...
该配置当前仅由 schema 校验，controller 尚未消费。
...
该配置当前仅由 schema 校验，controller 尚未执行短路扣分。
