...
仓库根目录不再放置 `SKILL.md`。安装和卸载由 README 与 `scripts/install-codex.sh`、`scripts/uninstall-codex.sh` 承担，避免把安装说明误暴露成可调用 skill。
...
`scholar_writing/resources/config/reference_registry.yaml` 是写作规则资料的索引。它把 `scholar_writing/resources/references/` 下的风格指南、国自然结构、去 AI 规则、句式模板，以及 `paper/` 下的论文结构、章节写作、审阅、venue/visual、英文润色和引用策略规则映射到 action、agent role、项目类型和审阅维度。
