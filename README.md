# ScholarWriting（学术写作助手）

ScholarWriting 的中文用户入口名是“学术写作助手”。它面向国自然申报书、论文初稿和学术材料成稿，帮助用户按“规划、写作、审阅、修订、再审阅”的顺序推进文档。

`scholar-writing` 是机器 ID、CLI 命令和安装目录名。日常使用时，普通用户直接说“学术写作助手”即可。
...
2. 在任意位置创建自己的写作项目。
3. 用 Codex 打开这个写作项目。
4. 在 Codex 里调用“学术写作助手”。

示例：

```text
使用学术写作助手帮我写一个国自然面上项目申报书。
...
```text
使用学术写作助手审阅并优化 ./my-proposal 里的申报书初稿。
```

助手会读取项目文件，判断下一步该规划、写作、审阅还是修订。底层 CLI 是给写作角色和开发者用的状态推进工具，普通用户不需要把它当作主要入口。

助手会自动加载写作规则资料。`scholar_writing/resources/references/` 中的风格指南、国自然结构规范、去 AI 痕迹规则、章节句式模板，以及 `paper/` 下的论文 lifecycle、章节写作、同行评审、venue/visual、英文润色和 citation strategy references 会按项目类型进入任务包，Codex 或 Claude Code 的写作角色按这些规则写作、审阅和修订。用户只需要提供自己的材料、提纲或初稿。
...
Codex 应按本 README 的安装说明执行仓库里的安装脚本。安装完成后，重启 Codex，让 Codex 重新发现 `scholar-writing` skill 和配套写作角色。
...
- Codex 全局 skill：学术写作助手（机器 ID：`scholar-writing`）
- 4 个 Codex 写作角色：architect、writer、reviewer、revision
- ScholarWriting 运行资源：Python controller、schemas、templates、references、prompts
- 包装命令：`bin/scholar-writing`

安装后重启 Codex，让 Codex 重新发现 skill 和写作角色。
...
卸载脚本会删除安装到 Codex 的 skill、运行资源和 `scholar-*` 写作角色。它不会删除你的写作项目目录，也不会删除你已经生成的申报书材料。
...
```text
使用学术写作助手创建一个国自然面上项目写作目录，目录名是 my-proposal。
```
...
```text
使用学术写作助手根据 ./my-proposal/planning/outline.md 开始逐章写作。
```
...
使用学术写作助手做多维度审阅，必要时自动修订；遇到核心论点或大结构调整时先问我。
```
...
安装后的 Codex skill 会使用安装目录里的运行资源和 references。你的写作项目只保存自己的材料、提纲、章节、审阅和修订结果。
...
```text
adapters/claude-code/skills/pipeline/
adapters/claude-code/skills/architect/
adapters/claude-code/skills/writer/nsfc/
adapters/claude-code/skills/reviewer/
adapters/claude-code/skills/revision/
```

使用方式同样应以自然语言为主：

```text
使用学术写作助手帮我基于 ./my-proposal/materials 写一份国自然申报书。
```

或：

```text
使用学术写作助手审阅优化 ./my-proposal 的已有初稿。
```

Claude Code 适配层的角色拆分更细，包含章节 writer、专项 reviewer、修订角色和专家模拟 reviewer。它适合沿用 Claude Code 的原生调度方式。

如果你同时使用 Codex 和 Claude Code，需要记住：

| 项目 | Codex | Claude Code |
| --- | --- | --- |
| 用户入口 | 自然语言触发“学术写作助手”（机器 ID：`scholar-writing`）| 自然语言触发“学术写作助手”（机器 ID：`scholar-writing`）|
| 角色粒度 | 4 类通用角色 | 更细的章节与审阅角色 |
...
不要求你一开始就准备完整目录。你可以只有素材、只有大纲，或者只有初稿。助手会根据已有文件判断入口。

## 自动优化循环

ScholarWriting 的循环会把生成任务和状态判断分开，避免写作角色凭感觉反复改：
...
普通用户不需要手动编辑 `scores.yaml`。如果流程中断，下次继续让助手处理同一个项目目录即可。
...
- 平台通用 repo-local skill 入口。
- 安装后的 Codex skill 入口。
- Codex 4 类写作角色。
- Claude Code 适配层。
- 项目状态文件和自动恢复机制。
- references 自动接入 taskpack，写作、审阅和修订会读取质量规则；paper 项目会额外读取论文结构、review、venue/visual、英文润色和引用策略规则。
- 从素材、提纲、初稿三种入口开始。
- 写作、审阅、修订的基础闭环。
- 国自然项目的章节结构和命名契约。
...
开发者可以直接在本仓库里调试平台通用 repo-local skill 和 Codex 角色定义：
...
这条路径用于开发和验证 skill/角色文案、taskpack 契约、references 接入和状态机行为。普通用户的日常写作仍建议使用本机安装后的流程。
...
任务包中的 `reference_inputs` 会列出本轮写作角色应读取的质量规则文件。新增规则资料时，开发者应维护 `scholar_writing/resources/config/reference_registry.yaml`，并确保路径仍为相对路径。
...
本项目最早围绕 Claude Code 的 Skill/Agent 机制设计，因此仓库里保留了较细的角色资产，包括 architect、多个 NSFC writer、多个 reviewer、revision 和 expert simulation。

Codex 适配先建立共享的项目文件契约、状态机、CLI、schema 和 taskpack，再用 Codex 的角色机制执行任务。

因此当前实现是：

- 共享核心：`scholar_writing/`，包括 Python controller、prompts、schemas、templates、reference registry、项目目录结构和 `scores.yaml`。
- 平台通用 repo-local skill：`.agents/skills/scholar-writing/`。
- Codex 适配：`.codex/agents/` 和安装脚本生成的 Codex 专用 skill。
- Claude Code 适配：`adapters/claude-code/skills/` 下的角色资产。

这也是为什么不同框架的角色数量、调度方式和并行方式不同，但可以处理同一类写作项目。
...
- [Schema 说明](scholar_writing/resources/schemas/README.md)
