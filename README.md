...
项目配置中的 `project.language` 只控制最终学术产物的语言和参考规则选择：`zh` 生成中文报告或论文，`en` 生成英文报告或论文。CLI、错误、进度和确认等交互始终使用简体中文，不随该字段切换。
...
助手会自动加载写作规则资料。`scholar_writing/resources/references/` 中的风格指南、国自然结构规范、去 AI 痕迹规则、章节句式模板，以及 `paper/` 下的论文 lifecycle、章节写作、同行评审、venue/visual、英文润色和 citation strategy references 会按项目类型进入任务包，Codex 写作角色按这些规则写作、审阅和修订。用户只需要提供自己的材料、提纲或初稿。
...
- 写作任务会加载中文学术风格、通用句式和国自然结构；任务包明确提供目标章节时，才额外加载对应分章节模板。
...
共享核心负责确定性状态推进和任务包生成，Codex adapter 负责把任务包交给对应写作角色。
...
- [References 接入设计记录（历史）](docs/reference-integration-redesign.md)
